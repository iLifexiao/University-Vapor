...
extension ShootAndPrintController {
    ...
    func getPageHandler(_ req: Request) throws -> Future<[ShootAndPrint]> {
        _ = try req.requireAuthenticated(APIUser.self)
        ...
        return ShootAndPrint.query(on: req).filter(\.status != 0).sort(\.createdAt, .descending).range(low..<up).all()
    }
